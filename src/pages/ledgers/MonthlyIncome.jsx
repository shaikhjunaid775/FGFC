...
const MonthlyIncome = () => {
  ...
  const initialData = [
    ...
    },
    {
      ...
      monthlyProfit: 3500,
      slab: "Gold",
      payoutDate: "2024-02-07",
      status: "Completed"
    },
    {
      id: 4,
      date: "2024-02-04 01:00 PM",
      particulars: "Trading Profit",
      monthlyProfit: 4500,
      slab: "Platinum",
      payoutDate: "2024-02-08",
      status: "Failed"
    },
    {
      id: 5,
      date: "2024-02-05 02:30 PM",
      particulars: "Referral Bonus",
      monthlyProfit: 1500,
      slab: "Bronze",
      payoutDate: "2024-02-09",
      status: "Completed"
    }
  ];
  ...
  const [filters, setFilters] = useState({
    slab: "All", // Changed from "club" to "slab"
    status: "All"
  });
  ...
  const uniqueSlabs = ["All", ...new Set(data.map((item) => item.slab))];
  ...
  useEffect(() => {
    ...
    // Apply slab filter
    if (filters.slab !== "All") {
      result = result.filter((item) => item.slab === filters.slab);
    }
      ...
      <div className="space-y-4 p-4 bg-gray-50 rounded-xl pb-20">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4 items-end">
            {/* slab Filter */}
            <div>
              <label className="block mb-1 text-sm font-medium">Slab:</label>
              <select
                className="border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full"
                value={filters.slab}
                onChange={(e) => handleFilterChange("slab", e.target.value)}
              >
                {uniqueSlabs.map((slab) => (
          ...
          <table className="w-full text-sm text-left text-gray-500">
            <thead className="text-gray-700  bg-gradient-to-r from-primary to-secondary">
              <tr className="text-center whitespace-nowrap capitalize">
                ...
                <th className="px-3 py-2">monthlyProfit</th>
